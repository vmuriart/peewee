...
class TestModelAPIs(ModelTestCase):
    ...
    def test_callable_related_name(self):
        ...
        def rel_name(field):
            return '{0!s}_{1!s}_ref'.format(field.model_class._meta.name,
                                            field.name)
    ...
    def test_counting(self):
        ...
        for u in [u1, u2]:
            for i in range(5):
                Blog.create(title='b-{0!s}-{1!s}'.format(u.username, i),
                            user=u)
            ...
            for user in User.select():
                for i in range(20):
                    Blog.create(user=user,
                                title='b-{0:d}-{1:d}'.format(user.id, i))
...
class TestDeleteRecursive(ModelTestCase):
    ...
    def test_recursive_non_pk_fk(self):
        for i in range(3):
            ...
            for j in range(4):
                PackageItem.create(package=str(i),
                                   title='{0!s}-{1!s}'.format(i, j))
