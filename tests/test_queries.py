...
class TestSelectQuery(PeeweeTestCase):
    ...
    def test_reselect(self):
        ...
        assert id(sq2) != id(sq3)

    def test_select_subquery(self):
        subquery = SelectQuery(Child, fn.Count(Child.id)).where(
            Child.parent == Parent.id).group_by(Child.parent)
    ...
    def test_select_cloning(self):
        ct = fn.Count(Blog.pk)
        sq = SelectQuery(User, User, User.id.alias('extra_id'),
                         ct.alias('blog_ct')).join(
    ...
    def test_join_switching(self):
        ...
        multiple_first = Track.select().join(ReleaseTrack).join(
            Release).switch(Track).join(Artist).switch(Track).join(
            TrackGenre).join(Genre)
    ...
    def test_where_functions(self):
        sq = SelectQuery(User).where(
            fn.Lower(fn.Substr(User.username, 0, 1)) == 'a')
        self.assertWhere(sq, '(Lower(Substr("users"."username", ?, ?)) = ?)',
                         [0, 1, 'a'])
    ...
    def test_where_subqueries(self):
        ...
        self.assertWhere(sq,
                         '(("blog"."pk" = ?) OR ("blog"."user_id" IN (SELECT "users"."id" FROM "users" AS users WHERE ("users"."username" IN (?, ?)))))',
                         [3, 'u1', 'u2'])
    ...
    def test_where_fk(self):
        ...
        sq = SelectQuery(PackageItem).where(
            PackageItem.package == Package(barcode='b1'))
    ...
    def test_where_negation(self):
        ...
        sq = SelectQuery(Blog).where(
            ~((Blog.title == 'foo') | (Blog.title == 'bar')))
        self.assertWhere(sq,
                         'NOT (("blog"."title" = ?) OR ("blog"."title" = ?))',
                         ['foo', 'bar'])
        ...
        sq = SelectQuery(Blog).where(
            ~((Blog.title == 'foo') & (Blog.title == 'bar')) & (
                Blog.title == 'baz'))
        self.assertWhere(sq,
                         '(NOT (("blog"."title" = ?) AND ("blog"."title" = ?)) AND ("blog"."title" = ?))',
                         ['foo', 'bar', 'baz'])

        sq = SelectQuery(Blog).where(
            ~((Blog.title == 'foo') & (Blog.title == 'bar')) & (
                (Blog.title == 'baz') & (Blog.title == 'fizz')))
        self.assertWhere(sq,
                         '(NOT (("blog"."title" = ?) AND ("blog"."title" = ?)) AND (("blog"."title" = ?) AND ("blog"."title" = ?)))',
                         ['foo', 'bar', 'baz', 'fizz'])
    ...
    def test_ordering(self):
        ...
        sq = SelectQuery(User).join(Blog).order_by(User.username.desc(),
                                                   Blog.title.asc())
        self.assertOrderBy(sq, '"users"."username" DESC, "blog"."title" ASC',
                           [])

        base_sq = SelectQuery(User, User.username,
                              fn.Count(Blog.pk).alias('count')).join(
            Blog).group_by(User.username)
    ...
    def test_ordering_extend(self):
        ...
        assert [f.name for f in sq._order_by] == ['username']

        sq = sq.order_by(User.id.desc(), extend=True)
        assert [f.name for f in sq._order_by] == ['username', 'id']

        sq = sq.order_by(extend=True)
        assert [f.name for f in sq._order_by] == ['username', 'id']

        sq = sq.order_by()
        assert sq._order_by is None

        sq = sq.order_by(extend=True)
        assert sq._order_by is None

        with pytest.raises(ValueError):
            sq.order_by(foo=True)
    ...
    def test_prefetch_subquery(self):
        ...
        fixed_sql = [
            (
                'SELECT "t1"."id", "t1"."username" FROM "users" AS t1 WHERE ("t1"."username" = ?)',
                ['foo']),
            (
                'SELECT "t1"."pk", "t1"."user_id", "t1"."title", "t1"."content", "t1"."pub_date" FROM "blog" AS t1 WHERE (("t1"."title" = ?) AND ("t1"."user_id" IN (SELECT "t2"."id" FROM "users" AS t2 WHERE ("t2"."username" = ?))))',
                ['bar', 'foo']),
            (
...
class TestInsertQuery(PeeweeTestCase):
    ...
    def test_on_conflict(self):
        ...
        sql, params = TestUser.insert(username='huey').on_conflict(
            'IGNORE').sql()
        assert sql == (
    ...
    def test_returning(self):
        ...
        test_db.returning_clause = False
        with pytest.raises(ValueError):
            iq.returning(User.id)

        test_db.returning_clause = True
        iq_returning = iq.returning(User.id)

        assert not (id(iq_returning) == id(iq))
...
class TestDeleteQuery(PeeweeTestCase):
    ...
    def test_returning(self):
        ...
        test_db.returning_clause = False
        with pytest.raises(ValueError):
            dq.returning(User.username)

        test_db.returning_clause = True
        dq_returning = dq.returning(User.username)

        assert not (id(dq_returning) == id(dq))
...
class TestDjangoFilters(PeeweeTestCase):
    ...
    def test_filter(self):
        ...
        sq = Blog.filter(user__username__in=['u1', 'u2'],
        ...
        sq = Blog.filter(user__username__in=['u1', 'u2']).filter(
            comments__comment='hurp')
        self.assertJoins(sq, [
            'INNER JOIN "users" AS users ON ("blog"."user_id" = "users"."id")',
            'INNER JOIN "comment" AS comment ON ("blog"."pk" = "comment"."blog_id")',
    ...
    def test_filter_dq(self):
        ...
        self.assertJoins(sq, [])
        self.assertWhere(sq,
                         '(("users"."username" = ?) OR ("users"."username" = ?))',
                         ['u1', 'u2'])

        sq = Comment.filter(
            DQ(blog__user__username='u1') | DQ(blog__title='b1'),
            DQ(comment='c1'))
        self.assertJoins(sq, [
    ...
    def test_annotate(self):
        ...
        self.assertSelect(sq,
                          '"users"."id", "users"."username", Count("blog"."pk") AS count',
                          [])
        self.assertJoins(sq, [
            'INNER JOIN "blog" AS blog ON ("users"."id" = "blog"."user_id")'])
        self.assertWhere(sq, '', [])
        ...
        sq = User.select(User.username).annotate(Blog, fn.Sum(Blog.pk).alias(
            'sum')).where(User.username == 'foo')
        ...
        sq = User.select(User.username).annotate(Blog).annotate(Blog, fn.Max(
            Blog.pk).alias('mx'))
        self.assertSelect(sq,
                          '"users"."username", Count("blog"."pk") AS count, Max("blog"."pk") AS mx',
                          [])
        self.assertJoins(sq, [
            'INNER JOIN "blog" AS blog ON ("users"."id" = "blog"."user_id")'])
        self.assertWhere(sq, '', [])
        ...
        sq = User.select().join(Blog, JOIN.LEFT_OUTER).switch(User).annotate(
            Blog)
        self.assertSelect(sq,
                          '"users"."id", "users"."username", Count("blog"."pk") AS count',
                          [])
        self.assertJoins(sq, [
            'LEFT OUTER JOIN "blog" AS blog ON ("users"."id" = "blog"."user_id")'])
        self.assertWhere(sq, '', [])
        self.assertGroupBy(sq, '"users"."id", "users"."username"', [])
...
class TestQueryCompiler(PeeweeTestCase):
    def test_clause(self):
        ...
        assert sql == 'extract(? FROM "pub_date")'
        assert params == ['year']
...
class TestProxy(PeeweeTestCase):
    ...
    def test_proxy_database(self):
        ...
        assert DummyModel.get().test_field == 'foo'
...
class TestForUpdate(ModelTestCase):
    ...
    def test_for_update(self):
        ...
        updated = User.update(username='u1_edited').where(
            User.username == 'u1').execute()
        assert updated == 1
